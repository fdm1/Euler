...
*																				*
********************************************************************************/
int compare(int current, int answer)
{
	current > answer ? answer = current: answer;
	return answer;
}
...
int main (void)
{
	...
	//calculate all horizontal options
	for (int i = 0; i < 20; i++)
	{
		for (int j = 0; j < 16; j++)
			{
				current = a[i][j] * a[i][j + 1] * a[i][j + 2] * a[i][j + 3]; 
				answer = compare(current, answer);
			}

	}

	//calculate all verticle options
	for (int i = 0; i < 16; i++)
	{
		for (int j = 0; j < 20; j++)
		{
	...
	//calculate all down-right diagonal options
	for (int i = 0; i < 16; i++)
	{
		for (int j = 0; j < 16; j++)
		{
	...
	//calculate all up-right diagonal options
	for (int i = 0; i < 16; i++)
	{
		for (int j = 0; j < 16; j++)
		{	
			current = a[i+ 3][j] * a[i + 2][j + 1] * a[i + 1][j + 2] * a[i][j + 3]; 
			answer = compare(current, answer);
		}
	}
